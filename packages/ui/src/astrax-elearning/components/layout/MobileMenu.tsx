...
export default function MobileMenu({ isMobileMenu, handleMobileMenu, menuItems = [] }: any) {
											...
											if (hasSubItems) {
												return (
													<li className="has-children" key={item.id}>
														<span className="menu-expand" onClick={() => handleAccordion(index + 1)}><i className="arrow-small-down" /></span>
														...
														<ul className="sub-menu" style={{ display: `${isAccordion == index + 1 ? "block" : "none"}` }}>
															{item.subItems.map((subItem: any) => {
																...
																return (
																	<li key={subItem.id}>
																		<Link href={subItemUrl}>{subItem.title}</Link>
																	</li>
																);
															})}
														</ul>
