...
class Funcionarios {
  public async getAllFuncionarios(params: IsearchParams): Promise<{ data: any[]; total: number }> {
    let search = '';
    ...
    try {
      ...
      const query = `
                SELECT 
                    id,
                    titular as titular,  
                    subarea as dependencia,
                    tipo_area as areaTipo,
                    parent_caption as area_tipo,
                    prefix as tratamiento,
                    orderby as orden
                FROM listado_funcionarios_vw 
