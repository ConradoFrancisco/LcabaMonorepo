...
export default function BasicTableOne({}) {
  ...
  const { data, offset, limit, setOffset, total } = useData<any>({
    getAll: (params) => MagazineService.getAll(params),
    loading,
    setLoading,
  ...
  return (
    <>
      <Search setSearch={setSearch} tipo={'publicacion'} setOffset={setOffset} />
                    ...
                    data?.map((row, index) => (
                      <TableRow key={index}>
                        {claves?.map((clave, subIndex) => (
                          <>
                            <TableCell key={subIndex} className="px-5 py-3 text-sm">
                              {clave === 'status' ? (
                                <Badge size="sm" color={row.status ? 'success' : 'warning'}>
                                  {row.status ? 'Activo' : 'Inactivo'}
                                </Badge>
                              ) : clave === 'fecha' ? (
                                formatDate(String(row[clave]))
                              ) : typeof row[clave] === 'object' &&
                                row[clave] !== null &&
                                'data' in (row[clave] as object) ? (
                                (row[clave] as { data?: unknown[] }).data?.[0] ? (
                                  String(
                                    (row[clave] as { data?: unknown[] }).data?.[0] === 1
                                      ? 'Sí'
                                      : 'No',
                                  )
                                ) : (
