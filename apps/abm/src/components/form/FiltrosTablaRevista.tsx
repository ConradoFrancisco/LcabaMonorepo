// Aclaraciones:
// -El usuario al colocar rangos en los filtros "feche hasta" y "fecha desde", van a compararse en la BBDD con el campo "date_ins".
...
export default function FiltrosTablaRevista({
  categorias,
  filtros,
  setFiltros,
  types,
  ...
  const setCategorias = (cats: string[]) => {
    setFiltros((prev) => ({ ...prev, categorias: cats.map(Number) }));
  };
            ...
            <div className="flex flex-wrap items-end gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 hover:cursor-pointer hover:text-black focus:ring-3 focus:outline-hidden xl:w-[230px] dark:border-gray-800 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30"
                />
              </div>
              {/* Fecha desde */}
              ...
              <div className="flex min-w-[200px] flex-col gap-1.5">
                ...
                <MultiSelect
                  ...
                  defaultSelected={filtros.categorias?.map(String) || []}
                  onChange={(val) => setCategorias(val)}
                  closeOnSelect={false}
                  searchable
