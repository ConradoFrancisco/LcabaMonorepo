'use client';
import React, { useState } from 'react';

interface SwitchProps {
  label: string;
  defaultChecked?: boolean;
  disabled?: boolean;
  onChange?: (checked: boolean) => void;
  color?: 'blue' | 'gray';
}

const Switch: React.FC<SwitchProps> = ({
  label,
  defaultChecked = false,
  disabled = false,
  onChange,
  color = 'blue',
}) => {
  const [isChecked, setIsChecked] = useState(defaultChecked);

  const handleToggle = () => {
    if (disabled) return;
    const newCheckedState = !isChecked;
    setIsChecked(newCheckedState);
    if (onChange) {
      onChange(newCheckedState);
    }
  };

  const switchColors =
    color === 'blue'
      ? {
          background: isChecked ? 'bg-brand-500' : 'bg-gray-200 dark:bg-white/10',
          knob: isChecked ? 'translate-x-full bg-white' : 'translate-x-0 bg-white',
        }
      : {
          background: isChecked ? 'bg-gray-800 dark:bg-white/10' : 'bg-gray-200 dark:bg-white/10',
          knob: isChecked ? 'translate-x-full bg-white' : 'translate-x-0 bg-white',
        };

  return (
    <div className="flex flex-col">
      {/* Label arriba */}
      <label
        className={`mb-2 text-sm font-medium ${
          disabled ? 'text-gray-400' : 'text-gray-700 dark:text-gray-400'
        }`}
      >
        {label}
      </label>

      {/* Switch */}
      <div
        className={`relative inline-flex h-6 w-11 cursor-pointer items-center rounded-full transition duration-150 ease-linear ${
          disabled ? 'pointer-events-none bg-gray-100 dark:bg-gray-800' : switchColors.background
        }`}
        onClick={handleToggle}
      >
        <span
          className={`shadow-theme-sm absolute top-0.5 left-0.5 h-5 w-5 transform rounded-full transition duration-150 ease-linear ${switchColors.knob}`}
        />
      </div>
    </div>
  );
};

export default Switch;
