...
export default function CategoriasPage() {
  ...
  const { data, offset, limit, setOffset, total, categorias, types, setFlag, setLimit } = useData({
  ...
  const { handleExportar, exportando } = useExportar({
    ...
    claves,
    nombreArchivo: 'Listado de publicaciones de Cultura',
    titulo: 'Listado de publicaciones de Cultura',
  });

  const handleDelete = async (id: any) => {
    try {
      await PostService.deletePost(id, 'cultura_');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  const handleStatusChange = async (id: any, newStatus: number) => {
    try {
      await PostService.updatePostStatus(id, newStatus, 'cultura_');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  return (
    <>
      ...
      <CulturaFormInModal isOpen={abierto} setOpen={setAbierto} setFlag={setFlag} flag={false} />
    </>
  );
}
