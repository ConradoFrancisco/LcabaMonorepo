...
export default function OipTiposPage() {
  ...
  const handleDelete = async (id: any) => {
    try {
      await TypesService.deleteType(id, 'evidencias_posts_type');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  const handleStatusChange = async (id: any, newStatus: number) => {
    try {
      await TypesService.updateStatus(id, newStatus, 'evidencias_posts_type');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  return (
    <>
      ...
      >
        <Search setSearch={setSearch} tipo={'OIP Tipos'} setOffset={setOffset} />
        <TableComponent
          section="/oip/tipos"
          data={data}
          loading={loading}
      ...
      <TiposFormInModal
        ...
        setFlag={setFlag}
        table="evidencias_posts_type"
        modalTitle="Crear nuevo tipo de informe"
        editRoute="/oip/tipos/edit"
      />
    </>
  );
}
