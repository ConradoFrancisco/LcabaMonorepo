...
export default function RevistaTiposPage() {
  ...
  const handleDelete = async (id: any) => {
    try {
      await TypesService.deleteType(id, 'magazine_posts_type');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  const handleStatusChange = async (id: any, newStatus: number) => {
    try {
      await TypesService.updateStatus(id, newStatus, 'magazine_posts_type');
      setFlag((prev) => !prev);
    } catch (error) {
  ...
  return (
    <>
      <ComponentCard
        title="Revista - Tipos de publicaciones"
        action={
          <button
            type="button"
      ...
      >
        <Search setSearch={setSearch} tipo={'Revista Tipos'} setOffset={setOffset} />
        <TableComponent
          section="/revista/tipos"
          data={data}
          loading={loading}
          total={total}
          limit={limit}
      ...
      <TiposFormInModal
        ...
        setFlag={setFlag}
        table="magazine_posts_type"
        modalTitle="Crear nuevo tipo de revista"
        editRoute="/revista/tipos/edit"
      />
    </>
  );
}
